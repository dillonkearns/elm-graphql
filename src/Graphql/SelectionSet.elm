...
functions that you can use to build up `SelectionSet`s for Objects, Interfaces,
and Unions in your GraphQL schema.

Note that in these examples, all of the modules under `Api.` are generated by running
the [`@dillonkearns/elm-graphql`](https://npmjs.com/package/@dillonkearns/elm-graphql)
command line tool.
